...
class EvalCallBack(Callback):
    ...
    def on_validation_batch_end(
        ...
        # self.val_vis[key].append(vis_item.reshape(-1, vis_item.shape[-1]))
        self.val_label[key].append(label_item.reshape(rute_item.shape[0], -1))
    ...
    def process_and_log_metrics(self, val_input, val_vis, val_label, trainer, val_idx, label_time):
        # import pdb; pdb.set_trace()
        
        fig_scatter = plot_scatter(val_vis, val_label)
        # fig_scatter_hyper = plot_scatter_hyper(val_vis, val_label)
        
        fig_scatter_time = plot_scatterRdBu(val_vis, label_time)
        ...
        log_dict = {
            ...
            f"{dataset_name}_fig_scatter_time": wandb.Image(fig_scatter_time),
    ...
    def get_rout_vis(self, tree_node_embedding, emb_vis, centre_node = np.array([-3, -1])):
        # import pdb; pdb.set_trace()
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        for val_idx in self.val_vis.keys():
            if True:
                ...
                gathered_label_all = np.concatenate(self.val_label[val_idx])
                gathered_label = gathered_label_all[:, 0]
                gathered_label_time = gathered_label_all[:, 1]
                ...
                if trainer.is_global_zero:
                    self.process_and_log_metrics(
                        gathered_input, gathered_vis, gathered_label, trainer, val_idx, gathered_label_time
                    )
                    ...
                    # )
                    if gathered_vis.shape[0] > 100000:
                        samplt_idx = np.random.choice(
                            gathered_vis.shape[0], 100000, replace=False
                        )
