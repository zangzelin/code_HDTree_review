...
class EvalCallBack(Callback):
    ...
    def rout_eval(self, trainer, pl_module, gathered_rout, gathered_label, emb_vis, gathered_sample, gathered_sample_history=None):
        # import pdb; pdb.set_trace()
        ...
        vis_plot_path = self.get_rout_vis(pl_module.tree_node_embedding, emb_vis)
        ...
        if gathered_sample_history is not None and gathered_sample_history.shape[1] == 784:
            print('Evaluating the routing vector, plotting the sample images...')
            gathered_image = gathered_sample_history.reshape(-1, 28, 28)
            import pdb; pdb.set_trace()
            # show 10*10 images
            fig_sample = plt.figure(figsize=(30, 30))
            for i in range(100):
            ...
            dict.update({"rout/fig_sample_image_history": wandb.Image(fig_sample)})
            plt.close()
