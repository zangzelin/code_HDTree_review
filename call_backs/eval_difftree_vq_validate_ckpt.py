...
class EvalCallBack(Callback):
    ...
    def process_and_log_metrics(self, val_input, val_vis, val_label, trainer, val_idx):
        ...
        elif val_vis.shape[0] > 5000:
            idx = np.random.choice(val_vis.shape[0], 5000, replace=False)
            val_input = val_input[idx]
            val_vis = val_vis[idx]
            val_label = val_label[idx]
        ...
        for val_idx in self.val_vis.keys():
            if True:
                ...
                if trainer.is_global_zero:
                    ...
                    elif gathered_vis.shape[0] > 5000:
                        samplt_idx = np.random.choice(
                            gathered_vis.shape[0], 5000, replace=False
                        )
                    else:
                        samplt_idx = np.arange(gathered_vis.shape[0])
                    ...
                    if True:
                        ...
                        dict_tree_result_list = []
                        acc_list = []
                        for num_cluster_use in range(num_clusters//2, num_clusters + 5):
                            
                            node_root = build_tree_zl(
                                gathered_rout, gathered_vector_rout, num_cluster_use
                            )
                            ...
                            acc_list.append(acc_cluster_acc)
                            dict_tree_result_list.append(dict_tree_result)
                            print(dict_tree_result)

                        wandb.log(dict_tree_result_list[np.argmax(acc_list)])
