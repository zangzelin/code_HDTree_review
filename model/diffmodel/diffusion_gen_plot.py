...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop_progressive_zl_step2(
            self,
            model,
            shape,
            device,
        ...
        for i in reversed(range(step)):
            ...
            img, pred_x0 = self.p_sample(model=model,
                                         x=img,
                                         t=torch.full((shape[0],), 0, dtype=torch.int64).to(device),
                                         noise_fn=noise_fn,
                                         return_pred_x0=True,
                                         lab=cond,
                                         )
    ...
    def p_sample_loop_progressive_simple(
        ...
        history.append(img.clone().clone().detach().cpu())
        for i in reversed(range(step//5)):
            ...
            history.append(img.clone().detach().cpu())
    ...
    #     history.append(img.clone())
    #     for i in reversed(range(step//4)):
    #         img, pred_x0 = self.p_sample(
    #             model=model,
    #             x=img,
    #             t=torch.full((shape[0],), i, dtype=torch.int64).to(device),
